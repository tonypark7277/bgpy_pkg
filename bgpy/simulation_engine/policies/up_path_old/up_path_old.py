...
#### filter counter
_filt_call_count_old = _mp_Value("q", 0)


def get_filt_call_count_old() -> int:
    """Return the total number of `filt` calls so far across all processes."""
    return _filt_call_count_old.value


def reset_filt_call_count_old() -> None:
    """Reset the global `filt` call counter back to zero."""
    with _filt_call_count_old.get_lock():
        _filt_call_count_old.value = 0
####
